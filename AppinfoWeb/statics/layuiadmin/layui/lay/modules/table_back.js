...
;layui.define(["laytpl", "laypage", "layer", "form"], function (e) {
    ...
    j.prototype.config = {limit: 30, loading: !0}, j.prototype.render = function () {
        ...
        if (n.url) t.ajax({
            ...
            dataType: "json",
            //添加token至header中
            headers: {"token_back":localStorage.getItem("token_back")},
            success: function (t) {
                register_back(t.code);
                if(t.code==801){
                    return;
                }
            ...
            complete: function( xhr,data ){
                if(xhr.getResponseHeader("reloadToken")!=null){
                    //置换token
                    localStorage.setItem("token_back",xhr.getResponseHeader("reloadToken"));
                }
            }
