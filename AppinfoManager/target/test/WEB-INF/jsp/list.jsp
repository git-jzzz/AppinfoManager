                ...
                <div class="layui-form-item">
                    <div style="margin-bottom: 5%;padding-top: 5%;margin-left: 5%" class="layui-inline">
                        <div class="layui-input-inlxvine">
                            <input type="text" name="softwareName" placeholder="软件名称" autocomplete="off"
                                   class="layui-input">
                        </div>
                    </div>
                    ...
                    <div class="layui-inline">
                        ...
                        <div class="layui-input-inline">
                            <select lay-filter="c1" id="3" name="categoryLevel3">
...
<div class="layui-card-body">
    <div style="padding-bottom: 10px;">
        <a href="sys/toappadd.html" class="layui-btn">新增app基础信息</a>
    </div>
    ...
    <script type="text/html" id="barDemo">
        ...
        <br/>
        <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="addb">新增版本</a>
        <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="updb">修改版本</a>
        {{#  if(d.statusName =="已上架"){ }}
        <a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="xia">下架</a>
        {{# }if(d.statusName =="已下架"||d.statusName =="审核通过") { }}
        <a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="shang">上架</a>
        {{#  } }}
    </script>
        ...
        layui.config({
            ...
            form.on('select(c1)', function (data) {
                ...
                //选择全部逻辑判断
                  ...
                  if(data.elem.getAttribute("id")==1){
                      $("[name=categoryLevel2]").html("");
                      $("[name=categoryLevel2]").append("<option value=''>全部</option>");
                      $("[name=categoryLevel3]").append("<option value=''>全部</option>");
                      }else{
                      $("[name=categoryLevel3]").append("<option value=''>全部</option>");
                  }
                  form.render('select');//select是固定写法 不是选择器   渲染 下拉框   否则不会刷新数据
        ...
        if(datas.length!=0){
          ...
          $("#" + id + "").append(" <option value=''>暂无</option>");
            ...
            table.on('tool(info)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'sel') {
                    location.href = "sys/findbyid.html?id=" + data.id;
                } else if (layEvent === 'del') {
                    layer.confirm('确认删除该APP吗?', function (index) {
                        $.getJSON("sys/deleteAppinfo.html", "appId=" + data.id, function (data) {
                    ...
                    location.href = "sys/toappversionadd.html?appId=" + data.id;
                } else if (layEvent === 'upd') {
                    if (data.statusName == ("审核未通过") || data.statusName == "待审核") {
                        location.href = "sys/findbyid.html?i=1&id=" + data.id;
                    } else {
                    ...
                    if (data.versionNo != null) {
                        if (data.statusName == ("审核未通过") || data.statusName == "待审核") {
                            location.href = "sys/toversionupdate.html?id=" + data.versionId + "&app=" + data.id;
                        } else {
                    ...
                    $.getJSON("sys/setStatus.html", "id=" + data.id + "&status=" + status, function (data) {
                        if (data.result = "success") {
                            //重载表格
                            table.reload('info', {
                                url: 'sys/list.html'
